...
Plugin URI: http://skullbit.com/wordpress-plugin/donate-plus
...
<strong><?php _e('Replacement Codes', 'dplus');?></strong> <small><?php _e('Email Message Only', 'dplus');?></small><br /><?php _e('<code>{donor}</code> = Donor Name<br \><code>{amount}</code> = Donation Amount<br \><code>{donorwall}</code> = Donor Wall URL','dplus');?></td>
		...
		function DonorWall($atts=false) {
			...
			foreach( $donors as $donor ):
				...
				$output .= '<div class="donorbox"><p><cite><strong><a href="'.$donor->url.'" rel="external" class="name url">'.$donor->name.'</a></strong> '.$donation.'</cite> '.__('Says:','dplus').' <br /><small class="date time"><a href="#donor-'.$donor->ID.'">'.$datetime.'</a></small><blockquote class="comment">'.nl2br($donor->comment).'</blockquote></div>';
			endforeach;
			$output .= '</div>';
			return $output;
		}
