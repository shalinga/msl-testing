...
//print_r($_POST);
            foreach ($_POST as $ipnvars => $ipnvalue) {$postvars[] = $ipnvars; $postvals[] = $ipnvalue;}
...
if( !$mc_gross ) $mc_gross = $payment_gross;
	...
	$postquery = "INSERT INTO ".$table_name." (name, email, url, comment, display, amount, currency, date, user_id, status, txn_id)
			VALUES( '$option_name1', '$option_selection1', '$url', '$option_selection2', '$display', '$mc_gross', '$mc_currency', '". date('Y-m-d H:i:s') ."', '$uID', '$payment_status', '$txn_id')";
...
/********************************************************
*    THIS SCRIPT IS FREEWARE AND IS NOT FOR RE-SALE     *
*   If you would like to have a custom script written   *
