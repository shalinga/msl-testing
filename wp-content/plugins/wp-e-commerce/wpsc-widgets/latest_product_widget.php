...
$post_id = $GLOBALS['post']->ID; //shalinga
		if 	($post_id !='1417'){
...
// link <bring up to by shalinga>
			$output .= '<div call="image_link">';
...
// Thumbnails, if required
			if ($image) {
				...
				$output .= '<div class="item_image new_item_image">';
...
//description Shalinga 17 oct 2011
			$output .= '<div class="image_description">';
			
		$pro_id = $latest_product->ID;
		$output_cont = get_post($pro_id, ARRAY_A);
		$output.= substr($output_cont['post_content'], 0, 70);
		$output.='...';
		$output.='<p>';
		$output.='Price: $ ';
		$output.= get_post_meta($pro_id, _wpsc_price, true);
		//$output.='.00';
			$output .= '</div>';// end image_description		
} 


//$output .= '<div class = "image_Link">'; // Shalinga 17 oct 2011
// Link
//$output .= '<a href="' . wpsc_product_url( $latest_product->ID, null ) . '" class="wpsc-product-title">'.stripslashes( //$latest_product->post_title ).'</a>';
//$output .= '</div>';
//$output .= '</div>';

 $action =  wpsc_product_external_link($pro_id); ?>
<?php echo $action; ?>
					
<?php		
			$output .= '<form class="product_form"  enctype="multipart/form-data" action="'.$action.'" method="post" name="product_' . $pro_id . '" id="product_' . $pro_id . '" >';
				$output .= '<input type="hidden" name="wpsc_ajax_action" value="add_to_cart" /> <input type="hidden" name="product_id" value="' . $pro_id . '" />';
				$output .= '<div class="wpsc_buy_button_container"><input id="product_' . $pro_id . '_submit_button" class="wpsc_buy_button" type="submit" name="Buy" value="Add To Cart" />';
				$output .= '<div class="wpsc_loading_animation" style="visibility: hidden;"><img title="Loading" src="/wp-content/plugins/wp-e-commerce/wpsc-theme/wpsc-images/indicator.gif" alt="Loading" />';
				$output .= 'Updating cart...</div>';
				$output .= '</div>';				
$output .= '</form>';	
$output .= '</li>';
		}
